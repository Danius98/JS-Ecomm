...
let serviceAccount = require("./html-css-js-ecomm-firebase-adminsdk-htyu4-3c6220fea0.json");
...
const bucketName = "dan-ecomm";
/*var credentials = new AWS.SharedIniFileCredentials({profile: 'dandu'});
...
app.post("/signup", (req, res) => {
    ...
    .then(user => {
        if(user.exists) {
            ...
            //encrypt the passwod before storing it.
...
app.post("/login", (req, res) => {
    ...
    if(!email.length || !password.length){
        return res.json({"alert": "fill all the inputs first"})
    }

    db.collection("users").doc(email).get()
    .then(user => {
        if(!user.exists){ // if email doesnt exists
            return res.json({"alert": "log in email done not exists"})
        } else {
...
app.post("/seller", (req, res) => {
    ...
    if(!name.length || !address.length || !about.length || number.length < 10 || !Number(number)){
        ...
        //update user sellers status here.
...
app.post('/add-product', (req, res) => {
    ...
    .catch(err => {
        return res.json({"alert": 'some error ocurred. Try again'});
    })
})
...
<body>
    ...
    let docName = email + Math.floor(Math.random() * 828342309);
    db.collection('order').doc(docName).set(req.body)
    .then(data => {
        transporter.sendMail(mailOption, (err, info) => {
            if(err){
                res.json({'alert': 'ops! it seems like some error occured. Try again'})
            } else {
